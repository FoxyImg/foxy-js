...
<template>
	<EditorPanel title="Mask" collapse-key="mask-editor" v-model="currentValue.enabled" :show-toggle="true" :disabled="!currentValue.enabled">
		...
		<OverlayImageValue v-if="currentValue.type === 'image'" title="Mask Image Key" default="" v-model="currentValue.imageKey" :overlay-images="overlayImages" @add-overlay-image="emit('addOverlayImage', $event)" @remove-overlay-image="emit('removeOverlayImage', $event)" />
