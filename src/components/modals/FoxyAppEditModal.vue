...
async function saveSource() {
	if (currentApp.value.id === null || currentApp.value.url === null || currentApp.value.secret === null) {
	...
	try {
		const res = await verifyAppId(currentApp.value.id, currentApp.value.url, currentApp.value.secret);
	...
	<ModalContainer v-bind="modalProps" @close="emit('close')">
		<div class="p-3 text-base flex flex-col gap-3">
			...
			<div class="flex flex-col gap-1">
				...
				<input type="password" v-model="currentApp.secret" class="border border-neutral-200 rounded-md px-2 py-1 text-sm">
			</div>
